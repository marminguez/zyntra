...
function normalizeLibreError(err: unknown): LibreSyncError {
  ...
  if (status === 401 || status === 403) {
    return new LibreSyncError(
      "LibreLink rechazó el acceso (401/403). Debes usar una cuenta LibreLinkUp que siga al paciente (invitación aceptada en LibreLinkUp). El acceso de \"aplicaciones conectadas\" en LibreView no reemplaza ese permiso. Verifica email/contraseña e inténtalo de nuevo.",
      status
    );
  }
